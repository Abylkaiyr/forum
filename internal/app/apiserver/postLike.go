...
func (c *APIServer) postLike(w http.ResponseWriter, r *http.Request) {
	userID, ok := r.Context().Value(keyUserId).(int)
	if userID == 0 {
		http.Redirect(w, r, "/", http.StatusSeeOther)
	}
	postId, err := strconv.Atoi(strings.TrimPrefix(r.URL.Path, "/post-like/"))
	if err != nil {
